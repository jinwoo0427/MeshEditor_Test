...
public class ModifyMesh : ScriptableObject, ISerializationCallbackReceiver
{
    ...
    public HandleRenderer handlesRenderer;   // 
    //public ModelSource source;                
    public string originalMeshGUID; 
    public Dictionary<int, int> triangleLookup = new Dictionary<int, int>();   
    ...
    [SerializeField] private Triangle[] _faces;
    [SerializeField] private Edge[] _edges;
    [SerializeField] private Edge[] _userEdges; 
    [SerializeField] private int _vertexCount;      
    [SerializeField] private int _triangleCount;    

    public Triangle[] faces { get { return _faces; } }
    public Edge[] edges { get { return _edges; } }
    public Edge[] userEdges { get { return _userEdges; } }
    ...
    public Triangle[] GetFaces()
    {
        ...
        //Triangle[] t = new Triangle[tris.Length / 3];
        //int index = 0;

        //for (int i = 0; i < tris.Length; i += 3)
        //{
        //    t[index++] = new Triangle(i, tris[i], tris[i + 1], tris[i + 2]);
        //}

        return TrianglesData.GetData(cloneMesh, cloneMesh.subMeshCount, 0);
    }
    ...
    public Edge[] GetEdges()
    {
        ...
        Edge[] edges = new Edge[tris.Length];

        for (int i = 0; i < tris.Length; i += 3)
        {
            edges[i + 0] = new Edge(tris[i + 0], tris[i + 1]);
            edges[i + 1] = new Edge(tris[i + 1], tris[i + 2]);
            edges[i + 2] = new Edge(tris[i + 2], tris[i + 0]);
        }
    ...
    public static ModifyMesh Create(GameObject InGameObject)
    {
        ModifyMesh qmesh = ScriptableObject.CreateInstance<ModifyMesh>();
    ...
    public void CacheElements()
    {
        ...
        bool[] assigned = MathHelper.FilledArray(false, vertexCount);
